...
once it is confident about them. The two ideas that make this cheap are (a) an
encoder that turns streaming audio into an incrementally-grown cross-attention
memory, and (b) a decoder that resumes from a frozen token prefix with its KV
tensors pinned on the NPU, so each preview is cheap.
...
```
feeder thread ──> audio_queue ──> worker thread ──> resample to 16 kHz
                                        │
...
Two threads: the **feeder thread** reads the WAV file and does
`audio_queue.put(block)` in fixed-size blocks (pacing itself to real time if
`--realtime` is passed); the **worker thread** does resample → VAD → encode →
decode → render. This decoupling mirrors what a live audio source would look
like and keeps the worker's timing measurements meaningful.

### Step by step

**1. Feed.** The WAV file (already loaded into memory via `soundfile`, with the
synthetic silence lead-in prepended) is pushed onto the queue in fixed
4096-sample blocks, simulating how a live audio source would deliver it.
...
the file's native sample rate; leftover samples carry to the next block.
...
**4. VAD** (`EnergyVAD`) — a self-calibrating RMS energy detector shared across a
speech/silence endpointing state machine (`_HangoverVAD`): per chunk it emits
`speech_start` / `speech` / `speech_end` (after `--vad-silence` s of quiet) /
`silence`. It samples the first ~12 chunks (~1 s, the synthetic lead-in — see
[Transcribing a WAV file](#transcribing-a-wav-file)) for ambient noise and sets
`threshold = max(mean + 4·std, --vad-threshold)`. A `--vad-lookback` chunks of 
pre-speech audio are buffered and replayed into the encoder on `speech_start` 
so word onsets aren't clipped by the encoder's warmup discard.
...
frames of lookahead, the last audio hasn't fully propagated when an utterance ends.
Finalize feeds **4 zero chunks** through the same path so the tail of the utterance
is pushed out of the lookahead pipeline (otherwise the last word or two is lost).

**7. When the decoder runs.** Four triggers in the worker:
...
4. **End of file** — input exhausted: flush + final decode for whatever utterance
   is still in flight, in case the file doesn't end in enough trailing silence for
   the VAD to have called `speech_end` itself.
...
must start a fresh utterance mid-file, so it locks the line, resets state, ticks
the counter, and redraws "Listening…" all at once before continuing.
