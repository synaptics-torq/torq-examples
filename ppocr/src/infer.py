...
def build_recognizer(args, char_dict, runtime_flags):
    ...
    if args.rec_vmfb:
        backend = NPUBackend(args.rec_vmfb, device_uri=args.device, runtime_flags=runtime_flags, device_io=args.device_io)
    ...
    for width in args.rec_buckets:
        ...
        backends[width] = NPUBackend(vmfb, device_uri=args.device, runtime_flags=runtime_flags, device_io=args.device_io)
...
def parse_args():
    ...
    parser.add_argument("--models", default=".", help="Directory holding the PP-OCR assets (default: %(default)s)")
    parser.add_argument("--device", default="torq")
    parser.add_argument("--no-refresh", action="store_true", help="Skip the model freshness check (offline runs)")
    ...
    parser.add_argument("--rec-buckets", type=int, nargs="+", default=list(DEFAULT_BUCKETS), help="Static widths available as bucket vmfbs (default: %(default)s)")
...
def main():
    ...
    try:
        ...
        print(f"Time: detection {stats['det_ms']:.1f}ms, recognition {stats['rec_ms']:.1f}ms ({stats['n_boxes']} boxes detected)")
