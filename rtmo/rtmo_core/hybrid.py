r"""Hybrid 3-vmfb runner: int8 backbone -> bf16 transformer -> int8 head, behind
the same ``.infer([input]) -> outputs`` / ``.infer_time_ms`` interface as
``torq.runtime.VMFBInferenceRunner``. All requant/transpose between the parts
happens host-side, exactly as it would on-device:
...
class HybridRunner:
    def __init__(self, backbone_vmfb, transformer_vmfb, head_vmfb, seams, *, device_uri="torq", function="main", torq_hw_type=None):
        # torq_hw_type only exists on newer (host) runners; pass through only when set.
        ...
        self.part_ms = {}
        # P3/P4 skips go int8->int8; when backbone-out scale/zp == head-in scale/zp
        # (same PTQ calibration) the dequant->requant round-trip is a no-op — skip it.
    ...
    @staticmethod
    def _dq(x, sz):
        return (x.astype(np.float32) - sz[1]) * sz[0]
    ...
    def infer(self, inputs):
        ...
        bb_out = self._bb.infer([inputs[0]])  # int8 NHWC image
        ...
        # P5 always requants (transformer changed it); P3/P4 pass through when fused.
        ...
        self.part_ms = {"backbone": (t1 - t0) * 1e3, "transformer": (t2 - t1) * 1e3, "head": (t3 - t2) * 1e3}
