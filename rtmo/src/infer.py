...
from rtmo.rtmo_core.quant import MODELS, dequantize_heads, quantize_input
...
def main():
    ...
    quantized = quantize_input(tensor, "hybrid")

    print("\nRunning inference (three chained vmfbs on the NPU)...")
    heads = dequantize_heads(runner.infer([quantized]), "hybrid")
    print(
